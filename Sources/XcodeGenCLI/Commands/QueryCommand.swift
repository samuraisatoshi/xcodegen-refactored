...
class QueryCommand: ProjectCommand {
    ...
    override func execute(specLoader: SpecLoader, projectSpecPath: Path, project: Project) throws {
        ...
        switch type {
        case .targets:
            let result = project.targets.map { TargetSummary(name: $0.name,
                                                              type: $0.type.name,
                                                              platform: $0.platform.rawValue) }
            success(try encode(result))

        case .target:
            let name = try requireName(for: type)
            guard let target = project.getTarget(name) else {
                throw QueryError.targetNotFound(name)
            }
